...
class CareerWorkers:
    def __init__(self, store, settings): self.store,self.settings,self.llm=store,settings,LLM()
    def discover(self):
    ...
    def process(self, profile):
        for job in self.store.jobs():
            ...
            self.store.update_job(job['id'],status=analysis['decision'].lower(),fit_score=analysis['fit_score'],ats_score=analysis['ats_score'],reasoning=analysis['reasoning'])
            self.store.log('analysis',analysis['reasoning'],job['id'])
            if analysis['decision']=='Apply' and analysis['fit_score']>=80:
                resume,letter=self.documents(job,profile,analysis)
                status='ready_for_submission'
                self.store.add_application(job['id'],status,resume,letter)
                self.store.log('documents',f'Created truthful tailored documents; status: {status}.',job['id'])
